/**
 * example-plugin-player — Player dice roller widget.
 *
 * Same dice buttons as the DM widget, but without the secret roll toggle.
 * All player rolls are always public.
 *
 * Custom element: <example-plugin-player>
 */
class ExamplePluginPlayer extends HTMLElement {
  constructor() {
    super()
  ...
  _roll(sides) {
    ...
    this.shadowRoot.getElementById('result').textContent = result
    this.shadowRoot.getElementById('result-label').textContent = `d${sides}`

    this.dispatchEvent(
      new CustomEvent('dice-roll', {
        bubbles: true,
        composed: true,
        detail: { sides, result, secret: false },
      })
    )
  }
}

customElements.define('example-plugin-player', ExamplePluginPlayer)
